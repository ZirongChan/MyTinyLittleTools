function [out] = equi2multimono_custom(im, out_size, fov)
...
% Default vertical FOV, and output size as input
if nargin == 2
    if size(out_size,1)==1
        out_width = out_size(1);
        out_height = out_width;
    elseif size(out_size,1)==2
        out_width = out_size(1);
        out_height = out_size(2);
    elseif size(out_size,1)==0 || size(out_size,1)>2
...
if nargin == 3
    ...
    elseif size(fov, 1)==2
        hfov = fov(1);
        vfov = fov(2);
    elseif size(fov,1)==0 || size(fov,1)>2
        error('Insufficient arguments');
    end
end
...
topLeft = [-tan(hfov/2), -tan(vfov/2), 1];

% Scaling factor for grabbing pixel co-ordinates
uv = [-2*topLeft(1)/out_width, -2*topLeft(2)/out_height, 0];
...
[X, Y] = meshgrid(0:out_width-1, 0:out_height-1);
X = X(:);
Y = Y(:);
